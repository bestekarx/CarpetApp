...
public class EntryRepository : IEntryRepository
{
    ...
    {
        if (!_str2type.ContainsKey(contentTypeString)) throw new ContentTypeStringUnknownException(contentTypeString);
    ...
    {
        if (!_type2str.ContainsKey(type)) throw new EntryTypeUnknownException(type);
    ...
    public async Task<List<Entities.Base.Entry>> FindEntriesByUuidsAsync(IEnumerable<Guid> uuids)
    {
        ...
        {
            var entry = await FindEntryByUuidAsync(uuid);
            if (entry != null) bag.Add(entry);
        });
    ...
    public dynamic GetRepository(Type type)
    {
        ...
        if (!_entryTypes.Contains(type)) ThrowHelper.ThrowArgumentException();
